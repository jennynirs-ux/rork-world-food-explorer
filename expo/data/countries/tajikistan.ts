import { Country } from '@/types';

export const tajikistan: Country = {
    id: 'tajikistan',
    name: {
      en: 'Tajikistan',
      sv: 'Tadzjikistan',
      es: 'Tayikistán',
      fr: 'Tadjikistan',
      de: 'Tadschikistan',
      it: 'Tagikistan',
      pl: 'Tadżykistan',
      nl: 'Tadzjikistan',
      pt: 'Tajiquistão',
    },
    code: 'TJ',
    continent: {
      en: 'Asia',
      sv: 'Asien',
      es: 'Asia',
      fr: 'Asie',
      de: 'Asien',
      it: 'Asia',
      pl: 'Azja',
      nl: 'Azië',
      pt: 'Ásia',
    },
    flag: '🇹🇯',
    landscapeImage: 'https://images.pexels.com/photos/18719794/pexels-photo-18719794.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=627&w=1200',
    description: {
      en: 'Tajikistan is a mountainous Central Asian country, often called the "Roof of the World", known for the Pamir Mountains, dramatic landscapes, and rich Silk Road heritage.',
      sv: 'Tadzjikistan är ett bergigt centralasiatiskt land, ofta kallat "Världens tak", känt för Pamirbergen, dramatiska landskap och rikt sidenvägsarv.',
      es: 'Tayikistán es un país montañoso de Asia Central, a menudo llamado el "techo del mundo", conocido por las montañas del Pamir, sus espectaculares paisajes y el rico patrimonio de la Ruta de la Seda.',
      fr: 'Le Tadjikistan est un pays montagneux d\'Asie centrale, souvent appelé le « Toit du monde », connu pour les montagnes du Pamir, ses paysages spectaculaires et le riche patrimoine de la Route de la Soie.',
      de: 'Tadschikistan ist ein gebirgiges zentralasiatisches Land, das oft als „Dach der Welt" bezeichnet wird und für das Pamir-Gebirge, seine dramatischen Landschaften und sein reiches Erbe an der Seidenstraße bekannt ist.',
      it: 'Tajikistan is a mountainous Central Asian country, often called the "Roof of the World", known for the Pamir Mountains, dramatic landscapes, and rich Silk Road heritage.',
      pl: 'Tajikistan is a mountainous Central Asian country, often called the "Roof of the World", known for the Pamir Mountains, dramatic landscapes, and rich Silk Road heritage.',
      nl: 'Tajikistan is a mountainous Central Asian country, often called the "Roof of the World", known for the Pamir Mountains, dramatic landscapes, and rich Silk Road heritage.',
      pt: 'Tajikistan is a mountainous Central Asian country, often called the "Roof of the World", known for the Pamir Mountains, dramatic landscapes, and rich Silk Road heritage.',
    },
    quickFacts: [
      {
        label: {
          en: 'Capital',
          sv: 'Kapital',
          es: 'Capital',
          fr: 'Capitale',
          de: 'Kapital',
          it: 'Capital',
          pl: 'Capital',
          nl: 'Capital',
          pt: 'Capital',
        },
        value: {
          en: 'Dushanbe',
          sv: 'Dushanbe',
          es: 'Dusambé',
          fr: 'Douchanbé',
          de: 'Duschanbe',
          it: 'Dushanbe',
          pl: 'Dushanbe',
          nl: 'Dushanbe',
          pt: 'Dushanbe',
        },
      },
      {
        label: {
          en: 'Population',
          sv: 'Befolkning',
          es: 'Población',
          fr: 'Population',
          de: 'Bevölkerung',
          it: 'Population',
          pl: 'Population',
          nl: 'Population',
          pt: 'Population',
        },
        value: {
          en: '9.8 million',
          sv: '9,8 miljoner',
          es: '9,8 millones',
          fr: '9,8 millions',
          de: '9,8 Millionen',
          it: '9.8 million',
          pl: '9.8 million',
          nl: '9.8 million',
          pt: '9.8 million',
        },
      },
      {
        label: {
          en: 'Official Language',
          sv: 'Officiellt språk',
          es: 'Idioma oficial',
          fr: 'Langue officielle',
          de: 'Amtssprache',
          it: 'Official Language',
          pl: 'Official Language',
          nl: 'Official Language',
          pt: 'Official Language',
        },
        value: {
          en: 'Tajik',
          sv: 'tadzjikiska',
          es: 'tayiko',
          fr: 'Tadjik',
          de: 'Tadschikisch',
          it: 'Tajik',
          pl: 'Tajik',
          nl: 'Tajik',
          pt: 'Tajik',
        },
      },
      {
        label: {
          en: 'Currency',
          sv: 'Valuta',
          es: 'Moneda',
          fr: 'Devise',
          de: 'Währung',
          it: 'Currency',
          pl: 'Currency',
          nl: 'Currency',
          pt: 'Currency',
        },
        value: {
          en: 'Tajikistani Somoni',
          sv: 'Tadzjikistanska Somoni',
          es: 'Somoni tayiko',
          fr: 'Somoni tadjik',
          de: 'Tadschikistanische Somoni',
          it: 'Tajikistani Somoni',
          pl: 'Tajikistani Somoni',
          nl: 'Tajikistani Somoni',
          pt: 'Tajikistani Somoni',
        },
      },
      {
        label: {
          en: 'Area',
          sv: 'Område',
          es: 'Área',
          fr: 'Superficie',
          de: 'Fläche',
          it: 'Area',
          pl: 'Area',
          nl: 'Area',
          pt: 'Area',
        },
        value: {
          en: '143,100 km²',
          sv: '143 100 km²',
          es: '143.100 km²',
          fr: '143 100 km²',
          de: '143.100 km²',
          it: '143,100 km²',
          pl: '143,100 km²',
          nl: '143,100 km²',
          pt: '143,100 km²',
        },
      },
    ],
    facts: [
      {
        en: 'Over 90% of Tajikistan is mountainous',
        sv: 'Över 90 % av Tadzjikistan är bergigt',
        es: 'Más del 90% de Tayikistán es montañoso',
        fr: 'Plus de 90 % du Tadjikistan est montagneux',
        de: 'Über 90 % Tadschikistans sind gebirgig',
        it: 'Over 90% of Tajikistan is mountainous',
        pl: 'Over 90% of Tajikistan is mountainous',
        nl: 'Over 90% of Tajikistan is mountainous',
        pt: 'Over 90% of Tajikistan is mountainous',
      },
      {
        en: 'The Pamir Mountains are among the highest in the world',
        sv: 'Pamirbergen är bland de högsta i världen',
        es: 'Las montañas del Pamir se encuentran entre las más altas del mundo',
        fr: 'Les montagnes du Pamir sont parmi les plus hautes du monde',
        de: 'Das Pamir-Gebirge gehört zu den höchsten der Welt',
        it: 'The Pamir Mountains are among the highest in the world',
        pl: 'The Pamir Mountains are among the highest in the world',
        nl: 'The Pamir Mountains are among the highest in the world',
        pt: 'The Pamir Mountains are among the highest in the world',
      },
      {
        en: 'Tajik is a variety of Persian (Farsi)',
        sv: 'Tadzjikiska är en variation av persiska (farsi)',
        es: 'El tayiko es una variedad del persa (farsi)',
        fr: 'Le tadjik est une variété de persan (farsi)',
        de: 'Tadschikisch ist eine Variante des Persischen (Farsi)',
        it: 'Tajik is a variety of Persian (Farsi)',
        pl: 'Tajik is a variety of Persian (Farsi)',
        nl: 'Tajik is a variety of Persian (Farsi)',
        pt: 'Tajik is a variety of Persian (Farsi)',
      },
      {
        en: 'The country has more than 1,000 rivers and lakes',
        sv: 'Landet har mer än 1 000 floder och sjöar',
        es: 'El país tiene más de 1.000 ríos y lagos',
        fr: 'Le pays compte plus de 1 000 rivières et lacs',
        de: 'Das Land hat mehr als 1.000 Flüsse und Seen',
        it: 'The country has more than 1,000 rivers and lakes',
        pl: 'The country has more than 1,000 rivers and lakes',
        nl: 'The country has more than 1,000 rivers and lakes',
        pt: 'The country has more than 1,000 rivers and lakes',
      },
      {
        en: 'The Pamir Highway is one of the highest international roads on Earth',
        sv: 'Pamir Highway är en av de högsta internationella vägarna på jorden',
        es: 'La autopista Pamir es una de las carreteras internacionales más altas del mundo',
        fr: 'L\'autoroute du Pamir est l\'une des routes internationales les plus hautes du monde',
        de: 'Der Pamir Highway ist eine der höchsten internationalen Straßen der Erde',
        it: 'The Pamir Highway is one of the highest international roads on Earth',
        pl: 'The Pamir Highway is one of the highest international roads on Earth',
        nl: 'The Pamir Highway is one of the highest international roads on Earth',
        pt: 'The Pamir Highway is one of the highest international roads on Earth',
      },
      {
        en: 'Tajikistan has a strong tradition of hospitality and communal meals',
        sv: 'Tadzjikistan har en stark tradition av gästfrihet och gemensamma måltider',
        es: 'Tayikistán tiene una fuerte tradición de hospitalidad y comidas comunitarias',
        fr: 'Le Tadjikistan a une forte tradition d\'hospitalité et de repas en commun',
        de: 'Tadschikistan hat eine starke Tradition der Gastfreundschaft und der gemeinsamen Mahlzeiten',
        it: 'Tajikistan has a strong tradition of hospitality and communal meals',
        pl: 'Tajikistan has a strong tradition of hospitality and communal meals',
        nl: 'Tajikistan has a strong tradition of hospitality and communal meals',
        pt: 'Tajikistan has a strong tradition of hospitality and communal meals',
      },
      {
        en: 'Many villages in the Pamirs preserve distinct languages and cultural practices',
        sv: 'Många byar i Pamirs bevarar distinkta språk och kulturella sedvänjor',
        es: 'Muchas aldeas del Pamir conservan lenguas y prácticas culturales distintas.',
        fr: 'De nombreux villages du Pamir préservent des langues et des pratiques culturelles distinctes',
        de: 'Viele Dörfer im Pamir bewahren unterschiedliche Sprachen und kulturelle Praktiken',
        it: 'Many villages in the Pamirs preserve distinct languages and cultural practices',
        pl: 'Many villages in the Pamirs preserve distinct languages and cultural practices',
        nl: 'Many villages in the Pamirs preserve distinct languages and cultural practices',
        pt: 'Many villages in the Pamirs preserve distinct languages and cultural practices',
      },
      {
        en: 'Handwoven textiles and embroidery are important folk arts in many regions',
        sv: 'Handvävda textilier och broderier är viktiga folkkonst i många regioner',
        es: 'Los textiles tejidos a mano y los bordados son artes populares importantes en muchas regiones',
        fr: 'Les textiles tissés à la main et la broderie sont des arts populaires importants dans de nombreuses régions',
        de: 'Handgewebte Textilien und Stickereien sind in vielen Regionen wichtige Volkskunst',
        it: 'Handwoven textiles and embroidery are important folk arts in many regions',
        pl: 'Handwoven textiles and embroidery are important folk arts in many regions',
        nl: 'Handwoven textiles and embroidery are important folk arts in many regions',
        pt: 'Handwoven textiles and embroidery are important folk arts in many regions',
      },
    ],
    foodCulture: {
      en: 'Tajik cuisine is hearty and communal, centered around rice, meat, bread, and dairy. Meals are often shared from large platters and reflect strong Persian and Central Asian influences, with staples like non (bread), soups, kebabs, and pilaf (osh).',
      sv: 'Tadzjikisk mat är rejäl och gemensamhetsbaserad, centrerad kring ris, kött, bröd och mejeriprodukter. Måltiderna delas ofta från stora fat och återspeglar starka persiska och centralasiatiska influenser, med basprodukter som non (bröd), soppor, kebab och pilaf (osh).',
      de: 'Die tadschikische Küche ist herzhaft und gemeinschaftlich, zentriert um Reis, Fleisch, Brot und Milchprodukte. Mahlzeiten werden oft von großen Tabletts geteilt und spiegeln starke persische und zentralasiatische Einflüsse wider, mit Grundnahrungsmitteln wie Non (Brot), Suppen, Kebabs und Pilaw (Osh).',
      fr: 'La cuisine tadjike est consistante et communautaire, centrée autour du riz, de la viande, du pain et des produits laitiers. Les repas sont souvent partagés dans de grands plats et reflètent de fortes influences persanes et d\'Asie centrale, avec des aliments de base comme le non (pain), les soupes, les kebabs et le pilaf (osh).',
      es: 'La cocina tayika es abundante y comunitaria, centrada en el arroz, la carne, el pan y los lácteos. Las comidas a menudo se comparten de grandes bandejas y reflejan fuertes influencias persas y de Asia Central, con alimentos básicos como non (pan), sopas, kebabs y pilaf (osh).',
      it: 'La cucina tagika è sostanziosa e comunitaria, centrata su riso, carne, pane e latticini. I pasti vengono spesso condivisi da grandi vassoi e riflettono forti influenze persiane e centroasiatiche, con alimenti base come non (pane), zuppe, kebab e pilaf (osh).',
      pl: 'Kuchnia tadżycka jest sycąca i wspólnotowa, skoncentrowana na ryżu, mięsie, chlebie i nabiale. Posiłki są często dzielone z dużych półmisków i odzwierciedlają silne wpływy perskie i środkowoazjatyckie, ze staplami takimi jak non (chleb), zupy, kebaby i pilaf (osh).',
      nl: 'De Tadzjiekse keuken is stevig en gemeenschappelijk, gericht op rijst, vlees, brood en zuivel. Maaltijden worden vaak gedeeld van grote schalen en weerspiegelen sterke Perzische en Centraal-Aziatische invloeden, met basisvoedsel zoals non (brood), soepen, kebabs en pilaf (osh).',
      pt: 'A culinária tajique é farta e comunitária, centrada em arroz, carne, pão e laticínios. As refeições são frequentemente compartilhadas de grandes travessas e refletem fortes influências persas e da Ásia Central, com básicos como non (pão), sopas, kebabs e pilaf (osh).',
    },
    history: [
      {
        year: '6th–4th Century BC',
        title: 'Achaemenid & Iranian Influence',
        description: {
          en: 'The region was influenced by ancient Iranian empires and Persianate culture, shaping language and identity.',
          sv: 'Regionen var influerad av forntida iranska imperier och persisk kultur, som formade språk och identitet.',
          es: 'La región fue influenciada por los antiguos imperios iraníes y la cultura persa, lo que dio forma al idioma y la identidad.',
          fr: 'La région a été influencée par les anciens empires iraniens et la culture persane, façonnant la langue et l\'identité.',
          de: 'Die Region wurde von alten iranischen Reichen und der persischen Kultur beeinflusst und prägte Sprache und Identität.',
          it: 'The region was influenced by ancient Iranian empires and Persianate culture, shaping language and identity.',
          pl: 'The region was influenced by ancient Iranian empires and Persianate culture, shaping language and identity.',
          nl: 'The region was influenced by ancient Iranian empires and Persianate culture, shaping language and identity.',
          pt: 'The region was influenced by ancient Iranian empires and Persianate culture, shaping language and identity.',
        },
      },
      {
        year: '329 BC',
        title: 'Hellenistic Period',
        description: {
          en: 'After Alexander the Great\'s campaigns, Hellenistic states and trade networks influenced parts of Central Asia.',
          sv: 'Efter Alexander den Stores kampanjer påverkade hellenistiska stater och handelsnätverk delar av Centralasien.',
          es: 'Después de las campañas de Alejandro Magno, los estados helenísticos y las redes comerciales influyeron en partes de Asia Central.',
          fr: 'Après les campagnes d'Alexandre le Grand, les États hellénistiques et les réseaux commerciaux ont influencé certaines parties de l'Asie centrale.',
          de: 'Nach den Feldzügen Alexanders des Großen beeinflussten hellenistische Staaten und Handelsnetzwerke Teile Zentralasiens.',
          it: 'After Alexander the Great\'s campaigns, Hellenistic states and trade networks influenced parts of Central Asia.',
          pl: 'After Alexander the Great\'s campaigns, Hellenistic states and trade networks influenced parts of Central Asia.',
          nl: 'After Alexander the Great\'s campaigns, Hellenistic states and trade networks influenced parts of Central Asia.',
          pt: 'After Alexander the Great\'s campaigns, Hellenistic states and trade networks influenced parts of Central Asia.',
        },
      },
      {
        year: '8th–10th Century',
        title: 'Silk Road & Cultural Flourishing',
        description: {
          en: 'Trade along Silk Road routes supported cities and scholarship, strengthening Persian-influenced culture in the region.',
          sv: 'Handel längs Silk Road-rutter stödde städer och stipendier, vilket stärkte den persisk influerade kulturen i regionen.',
          es: 'El comercio a lo largo de las rutas de la Ruta de la Seda apoyó a las ciudades y la erudición, fortaleciendo la cultura de influencia persa en la región.',
          fr: 'Le commerce le long des routes de la Route de la Soie soutenait les villes et l\'érudition, renforçant ainsi la culture d\'influence persane dans la région.',
          de: 'Der Handel entlang der Seidenstraßen unterstützte Städte und Wissenschaft und stärkte die persisch geprägte Kultur in der Region.',
          it: 'Trade along Silk Road routes supported cities and scholarship, strengthening Persian-influenced culture in the region.',
          pl: 'Trade along Silk Road routes supported cities and scholarship, strengthening Persian-influenced culture in the region.',
          nl: 'Trade along Silk Road routes supported cities and scholarship, strengthening Persian-influenced culture in the region.',
          pt: 'Trade along Silk Road routes supported cities and scholarship, strengthening Persian-influenced culture in the region.',
        },
      },
      {
        year: '1929',
        title: 'Tajik SSR',
        description: {
          en: 'Tajikistan became a Soviet republic, bringing major social, economic, and infrastructure changes.',
          sv: 'Tadzjikistan blev en sovjetrepublik, vilket medförde stora sociala, ekonomiska och infrastrukturförändringar.',
          es: 'Tayikistán se convirtió en una república soviética, lo que trajo importantes cambios sociales, económicos y de infraestructura.',
          fr: 'Le Tadjikistan est devenu une république soviétique, entraînant d\'importants changements sociaux, économiques et infrastructurels.',
          de: 'Tadschikistan wurde eine Sowjetrepublik, was große soziale, wirtschaftliche und infrastrukturelle Veränderungen mit sich brachte.',
          it: 'Tajikistan became a Soviet republic, bringing major social, economic, and infrastructure changes.',
          pl: 'Tajikistan became a Soviet republic, bringing major social, economic, and infrastructure changes.',
          nl: 'Tajikistan became a Soviet republic, bringing major social, economic, and infrastructure changes.',
          pt: 'Tajikistan became a Soviet republic, bringing major social, economic, and infrastructure changes.',
        },
      },
      {
        year: '1991',
        title: 'Independence',
        description: {
          en: 'Tajikistan became independent following the dissolution of the Soviet Union.',
          sv: 'Tadzjikistan blev självständigt efter Sovjetunionens upplösning.',
          es: 'Tayikistán se independizó tras la disolución de la Unión Soviética.',
          fr: 'Le Tadjikistan est devenu indépendant après la dissolution de l\'Union soviétique.',
          de: 'Tadschikistan wurde nach der Auflösung der Sowjetunion unabhängig.',
          it: 'Tajikistan became independent following the dissolution of the Soviet Union.',
          pl: 'Tajikistan became independent following the dissolution of the Soviet Union.',
          nl: 'Tajikistan became independent following the dissolution of the Soviet Union.',
          pt: 'Tajikistan became independent following the dissolution of the Soviet Union.',
        },
      },
    ],
    innovations: [
      {
        name: {
          en: 'Persianate Poetry & Literature',
          sv: 'Persiska poesi och litteratur',
          es: 'Poesía y literatura persiate',
          fr: 'Poésie et littérature persanes',
          de: 'Persische Poesie und Literatur',
          it: 'Persianate Poetry & Literature',
          pl: 'Persianate Poetry & Literature',
          nl: 'Persianate Poetry & Literature',
          pt: 'Persianate Poetry & Literature',
        },
        year: '10th–15th Century',
        description: {
          en: 'Tajik (Persian) literary traditions form a major part of the region\'s cultural identity, with poetry central to education and celebrations.',
          sv: 'Tadzjikiska (persiska) litterära traditioner utgör en stor del av regionens kulturella identitet, med poesi centralt för utbildning och firande.',
          es: 'Las tradiciones literarias tayikas (persas) forman una parte importante de la identidad cultural de la región, y la poesía es fundamental para la educación y las celebraciones.',
          fr: 'Les traditions littéraires tadjikes (persanes) constituent une part importante de l'identité culturelle de la région, la poésie étant au cœur de l'éducation et des célébrations.',
          de: 'Tadschikische (persische) literarische Traditionen bilden einen wichtigen Teil der kulturellen Identität der Region, wobei die Poesie für Bildung und Feiern von zentraler Bedeutung ist.',
          it: 'Tajik (Persian) literary traditions form a major part of the region\'s cultural identity, with poetry central to education and celebrations.',
          pl: 'Tajik (Persian) literary traditions form a major part of the region\'s cultural identity, with poetry central to education and celebrations.',
          nl: 'Tajik (Persian) literary traditions form a major part of the region\'s cultural identity, with poetry central to education and celebrations.',
          pt: 'Tajik (Persian) literary traditions form a major part of the region\'s cultural identity, with poetry central to education and celebrations.',
        },
      },
      {
        name: {
          en: 'Mountain Irrigation & Terracing',
          sv: 'Bergsbevattning och terrasser',
          es: 'Riego y terrazas de montaña',
          fr: 'Irrigation et terrassement de montagne',
          de: 'Bergbewässerung und Terrassenbau',
          it: 'Mountain Irrigation & Terracing',
          pl: 'Mountain Irrigation & Terracing',
          nl: 'Mountain Irrigation & Terracing',
          pt: 'Mountain Irrigation & Terracing',
        },
        year: 'Centuries',
        description: {
          en: 'Communities developed practical agricultural techniques for steep terrain, including channels and terraced fields in valleys.',
          sv: 'Samhällen utvecklade praktiska jordbrukstekniker för brant terräng, inklusive kanaler och terrasserade fält i dalar.',
          es: 'Las comunidades desarrollaron técnicas agrícolas prácticas para terrenos escarpados, incluidos canales y campos en terrazas en los valles.',
          fr: 'Les communautés ont développé des techniques agricoles pratiques pour les terrains escarpés, notamment les canaux et les champs en terrasses dans les vallées.',
          de: 'Die Gemeinden entwickelten praktische landwirtschaftliche Techniken für steiles Gelände, einschließlich Kanälen und Terrassenfeldern in Tälern.',
          it: 'Communities developed practical agricultural techniques for steep terrain, including channels and terraced fields in valleys.',
          pl: 'Communities developed practical agricultural techniques for steep terrain, including channels and terraced fields in valleys.',
          nl: 'Communities developed practical agricultural techniques for steep terrain, including channels and terraced fields in valleys.',
          pt: 'Communities developed practical agricultural techniques for steep terrain, including channels and terraced fields in valleys.',
        },
      },
      {
        name: {
          en: 'Traditional Textile Arts',
          sv: 'Traditionell textilkonst',
          es: 'Artes textiles tradicionales',
          fr: 'Arts textiles traditionnels',
          de: 'Traditionelle Textilkunst',
          it: 'Traditional Textile Arts',
          pl: 'Traditional Textile Arts',
          nl: 'Traditional Textile Arts',
          pt: 'Traditional Textile Arts',
        },
        year: 'Centuries',
        description: {
          en: 'Embroidery, suzani-style patterns, and regional weaving traditions remain important cultural crafts and local economies.',
          sv: 'Broderier, mönster i suzanistil och regionala vävtraditioner är fortfarande viktiga kulturhantverk och lokala ekonomier.',
          es: 'Los bordados, los patrones de estilo suzani y las tradiciones regionales de tejido siguen siendo importantes artesanías culturales y economías locales.',
          fr: 'La broderie, les motifs de style suzani et les traditions régionales de tissage restent des métiers culturels et économiques locaux importants.',
          de: 'Stickereien, Muster im Susani-Stil und regionale Webtraditionen bleiben wichtige kulturelle Handwerke und die lokale Wirtschaft.',
          it: 'Embroidery, suzani-style patterns, and regional weaving traditions remain important cultural crafts and local economies.',
          pl: 'Embroidery, suzani-style patterns, and regional weaving traditions remain important cultural crafts and local economies.',
          nl: 'Embroidery, suzani-style patterns, and regional weaving traditions remain important cultural crafts and local economies.',
          pt: 'Embroidery, suzani-style patterns, and regional weaving traditions remain important cultural crafts and local economies.',
        },
      },
    ],
    mustVisit: [
      {
        name: {
          en: 'Pamir Highway (M41)',
          sv: 'Pamir Highway (M41)',
          es: 'Autopista del Pamir (M41)',
          fr: 'Autoroute du Pamir (M41)',
          de: 'Pamir Highway (M41)',
          it: 'Pamir Highway (M41)',
          pl: 'Pamir Highway (M41)',
          nl: 'Pamir Highway (M41)',
          pt: 'Pamir Highway (M41)',
        },
        description: {
          en: 'Legendary high-altitude route through dramatic mountain landscapes and remote villages.',
          sv: 'Legendarisk höghöjdsrutt genom dramatiska bergslandskap och avlägsna byar.',
          es: 'Ruta legendaria de gran altitud a través de espectaculares paisajes montañosos y pueblos remotos.',
          fr: 'Itinéraire légendaire en haute altitude à travers des paysages de montagne spectaculaires et des villages isolés.',
          de: 'Legendäre Höhenroute durch dramatische Berglandschaften und abgelegene Dörfer.',
          it: 'Legendary high-altitude route through dramatic mountain landscapes and remote villages.',
          pl: 'Legendary high-altitude route through dramatic mountain landscapes and remote villages.',
          nl: 'Legendary high-altitude route through dramatic mountain landscapes and remote villages.',
          pt: 'Legendary high-altitude route through dramatic mountain landscapes and remote villages.',
        },
      },
      {
        name: {
          en: 'Iskanderkul Lake',
          sv: 'Iskanderkul sjön',
          es: 'Lago Iskanderkul',
          fr: 'Lac Iskanderkoul',
          de: 'Iskanderkul-See',
          it: 'Iskanderkul Lake',
          pl: 'Iskanderkul Lake',
          nl: 'Iskanderkul Lake',
          pt: 'Iskanderkul Lake',
        },
        description: {
          en: 'A striking alpine lake in the Fann Mountains, linked to legends of Alexander (Iskander).',
          sv: 'En slående alpin sjö i Fannbergen, kopplad till legender om Alexander (Iskander).',
          es: 'Un impresionante lago alpino en las montañas Fann, vinculado a las leyendas de Alejandro (Iskander).',
          fr: 'Un lac alpin saisissant dans les monts Fann, lié aux légendes d\'Alexandre (Iskander).',
          de: 'Ein markanter Alpensee im Fann-Gebirge, der mit Legenden über Alexander (Iskander) verbunden ist.',
          it: 'A striking alpine lake in the Fann Mountains, linked to legends of Alexander (Iskander).',
          pl: 'A striking alpine lake in the Fann Mountains, linked to legends of Alexander (Iskander).',
          nl: 'A striking alpine lake in the Fann Mountains, linked to legends of Alexander (Iskander).',
          pt: 'A striking alpine lake in the Fann Mountains, linked to legends of Alexander (Iskander).',
        },
      },
      {
        name: {
          en: 'Fann Mountains',
          sv: 'Fannbergen',
          es: 'Montañas Fann',
          fr: 'Montagnes Fann',
          de: 'Fann-Gebirge',
          it: 'Fann Mountains',
          pl: 'Fann Mountains',
          nl: 'Fann Mountains',
          pt: 'Fann Mountains',
        },
        description: {
          en: 'Popular for hiking with turquoise lakes, jagged peaks, and scenic valleys.',
          sv: 'Populärt för vandring med turkosa sjöar, taggiga toppar och natursköna dalar.',
          es: 'Popular para practicar senderismo con lagos color turquesa, picos escarpados y valles pintorescos.',
          fr: 'Populaire pour la randonnée avec des lacs turquoise, des pics déchiquetés et des vallées pittoresques.',
          de: 'Beliebt zum Wandern mit türkisfarbenen Seen, schroffen Gipfeln und malerischen Tälern.',
          it: 'Popular for hiking with turquoise lakes, jagged peaks, and scenic valleys.',
          pl: 'Popular for hiking with turquoise lakes, jagged peaks, and scenic valleys.',
          nl: 'Popular for hiking with turquoise lakes, jagged peaks, and scenic valleys.',
          pt: 'Popular for hiking with turquoise lakes, jagged peaks, and scenic valleys.',
        },
      },
      {
        name: {
          en: 'Dushanbe',
          sv: 'Dushanbe',
          es: 'Dusambé',
          fr: 'Douchanbé',
          de: 'Duschanbe',
          it: 'Dushanbe',
          pl: 'Dushanbe',
          nl: 'Dushanbe',
          pt: 'Dushanbe',
        },
        description: {
          en: 'The capital city with parks, museums, bazaars, and a gateway to the mountains.',
          sv: 'Huvudstaden med parker, museer, basarer och en port till bergen.',
          es: 'La capital con parques, museos, bazares y una puerta de entrada a las montañas.',
          fr: 'La capitale avec des parcs, des musées, des bazars et une porte d\'entrée vers les montagnes.',
          de: 'Die Hauptstadt mit Parks, Museen, Basaren und einem Tor zu den Bergen.',
          it: 'The capital city with parks, museums, bazaars, and a gateway to the mountains.',
          pl: 'The capital city with parks, museums, bazaars, and a gateway to the mountains.',
          nl: 'The capital city with parks, museums, bazaars, and a gateway to the mountains.',
          pt: 'The capital city with parks, museums, bazaars, and a gateway to the mountains.',
        },
      },
      {
        name: {
          en: 'Wakhan Valley (near the Afghan border)',
          sv: 'Wakhan Valley (nära den afghanska gränsen)',
          es: 'Valle de Wakhan (cerca de la frontera con Afganistán)',
          fr: 'Vallée du Wakhan (près de la frontière afghane)',
          de: 'Wakhan-Tal (nahe der afghanischen Grenze)',
          it: 'Wakhan Valley (near the Afghan border)',
          pl: 'Wakhan Valley (near the Afghan border)',
          nl: 'Wakhan Valley (near the Afghan border)',
          pt: 'Wakhan Valley (near the Afghan border)',
        },
        description: {
          en: 'A culturally rich high valley known for sweeping views, fort ruins, and unique Pamiri heritage (access depends on permits/routes).',
          sv: 'En kulturellt rik hög dal känd för vidsträckta vyer, fortruiner och unika Pamiri-arv (tillgång beror på tillstånd/rutter).',
          es: 'Un valle alto culturalmente rico conocido por sus vistas panorámicas, ruinas de fuertes y un patrimonio Pamiri único (el acceso depende de permisos/rutas).',
          fr: 'Une haute vallée culturellement riche connue pour ses vues panoramiques, ses ruines de fort et son patrimoine unique du Pamiri (l\'accès dépend des permis/itinéraires).',
          de: 'Ein kulturell reiches Hochtal, bekannt für atemberaubende Ausblicke, Festungsruinen und das einzigartige Pamiri-Erbe (Zugang abhängig von Genehmigungen/Routen).',
          it: 'A culturally rich high valley known for sweeping views, fort ruins, and unique Pamiri heritage (access depends on permits/routes).',
          pl: 'A culturally rich high valley known for sweeping views, fort ruins, and unique Pamiri heritage (access depends on permits/routes).',
          nl: 'A culturally rich high valley known for sweeping views, fort ruins, and unique Pamiri heritage (access depends on permits/routes).',
          pt: 'A culturally rich high valley known for sweeping views, fort ruins, and unique Pamiri heritage (access depends on permits/routes).',
        },
      },
    ],
    travelEssentials: [
      {
        item: 'Best time to visit',
        description: {
          en: 'Late spring to early autumn (May–October) for most regions; Pamirs are best in summer.',
          sv: 'Sen vår till tidig höst (maj–oktober) för de flesta regioner; Pamirs är bäst på sommaren.',
          es: 'Desde finales de primavera hasta principios de otoño (mayo-octubre) en la mayoría de las regiones; Los Pamir son mejores en verano.',
          fr: 'Fin du printemps jusqu\'au début de l\'automne (mai-octobre) pour la plupart des régions ; Les Pamirs sont meilleurs en été.',
          de: 'Spätfrühling bis Frühherbst (Mai–Oktober) für die meisten Regionen; Pamirs sind im Sommer am besten.',
          it: 'Late spring to early autumn (May–October) for most regions; Pamirs are best in summer.',
          pl: 'Late spring to early autumn (May–October) for most regions; Pamirs are best in summer.',
          nl: 'Late spring to early autumn (May–October) for most regions; Pamirs are best in summer.',
          pt: 'Late spring to early autumn (May–October) for most regions; Pamirs are best in summer.',
        },
      },
      {
        item: 'Permits & borders',
        description: {
          en: 'Some areas (especially in GBAO/Pamirs) may require additional permits—check requirements in advance.',
          sv: 'Vissa områden (särskilt i GBAO/Pamirs) kan kräva ytterligare tillstånd – kontrollera kraven i förväg.',
          es: 'Algunas áreas (especialmente en GBAO/Pamir) pueden requerir permisos adicionales; verifique los requisitos con anticipación.',
          fr: 'Certaines zones (en particulier dans le GBAO/Pamir) peuvent nécessiter des permis supplémentaires : vérifiez les exigences à l\'avance.',
          de: 'Für einige Gebiete (insbesondere in GBAO/Pamirs) sind möglicherweise zusätzliche Genehmigungen erforderlich – überprüfen Sie die Anforderungen im Voraus.',
          it: 'Some areas (especially in GBAO/Pamirs) may require additional permits—check requirements in advance.',
          pl: 'Some areas (especially in GBAO/Pamirs) may require additional permits—check requirements in advance.',
          nl: 'Some areas (especially in GBAO/Pamirs) may require additional permits—check requirements in advance.',
          pt: 'Some areas (especially in GBAO/Pamirs) may require additional permits—check requirements in advance.',
        },
      },
      {
        item: 'Transport',
        description: {
          en: 'Shared taxis and 4x4s are common for mountain routes; roads can be slow and weather-dependent.',
          sv: 'Delade taxibilar och fyrhjulingar är vanliga för bergsvägar; vägar kan vara långsamma och väderberoende.',
          es: 'Los taxis compartidos y los 4x4 son habituales para las rutas de montaña; las carreteras pueden ser lentas y depender del clima.',
          fr: 'Les taxis partagés et les 4x4 sont courants pour les itinéraires de montagne ; les routes peuvent être lentes et dépendantes des conditions météorologiques.',
          de: 'Für Bergstrecken sind Sammeltaxis und Geländewagen üblich. Straßen können langsam und wetterabhängig sein.',
          it: 'Shared taxis and 4x4s are common for mountain routes; roads can be slow and weather-dependent.',
          pl: 'Shared taxis and 4x4s are common for mountain routes; roads can be slow and weather-dependent.',
          nl: 'Shared taxis and 4x4s are common for mountain routes; roads can be slow and weather-dependent.',
          pt: 'Shared taxis and 4x4s are common for mountain routes; roads can be slow and weather-dependent.',
        },
      },
      {
        item: 'Altitude',
        description: {
          en: 'Take time to acclimatize in high regions like the Pamirs; hydrate and plan easy days early on.',
          sv: 'Ta dig tid att acklimatisera dig i höga regioner som Pamirs; återfukta och planera enkla dagar tidigt.',
          es: 'Tómese el tiempo para aclimatarse en regiones altas como el Pamir; Hidrátate y planifica días tranquilos desde el principio.',
          fr: 'Prenez le temps de vous acclimater dans les régions élevées comme le Pamir ; hydratez-vous et planifiez des journées faciles dès le début.',
          de: 'Nehmen Sie sich Zeit, um sich in hochgelegenen Regionen wie dem Pamir zu akklimatisieren. Hydratieren Sie und planen Sie frühzeitig entspannte Tage.',
          it: 'Take time to acclimatize in high regions like the Pamirs; hydrate and plan easy days early on.',
          pl: 'Take time to acclimatize in high regions like the Pamirs; hydrate and plan easy days early on.',
          nl: 'Take time to acclimatize in high regions like the Pamirs; hydrate and plan easy days early on.',
          pt: 'Take time to acclimatize in high regions like the Pamirs; hydrate and plan easy days early on.',
        },
      },
      {
        item: 'Cash',
        description: {
          en: 'Carry somoni in rural areas; card acceptance can be limited outside major cities.',
          sv: 'Bär somoni på landsbygden; kortacceptans kan begränsas utanför större städer.',
          es: 'Llevar somoni en las zonas rurales; La aceptación de tarjetas puede estar limitada fuera de las principales ciudades.',
          fr: 'Transporter du somoni dans les zones rurales ; l\'acceptation des cartes peut être limitée en dehors des grandes villes.',
          de: 'Tragen Sie Somoni in ländlichen Gebieten; Außerhalb von Großstädten kann die Kartenakzeptanz eingeschränkt sein.',
          it: 'Carry somoni in rural areas; card acceptance can be limited outside major cities.',
          pl: 'Carry somoni in rural areas; card acceptance can be limited outside major cities.',
          nl: 'Carry somoni in rural areas; card acceptance can be limited outside major cities.',
          pt: 'Carry somoni in rural areas; card acceptance can be limited outside major cities.',
        },
      },
    ],
    mainDish: {
      id: 'tajikistan-main',
      name: {
        en: 'Osh (Plov)',
        sv: 'Osh (Plov)',
        es: 'Osh (Plov)',
        fr: 'Och (Plov)',
        de: 'Osh (Plov)',
        it: 'Osh (Plov)',
        pl: 'Osh (Plov)',
        nl: 'Osh (Plov)',
        pt: 'Osh (Plov)',
      },
      description: {
        en: 'Traditional pilaf made with rice, meat, carrots, and spices',
        sv: 'Traditionell pilaff gjord med ris, kött, morötter och kryddor',
        es: 'Pilaf tradicional elaborado con arroz, carne, zanahorias y especias',
        fr: 'Pilaf traditionnel à base de riz, de viande, de carottes et d\'épices',
        de: 'Traditioneller Pilaw aus Reis, Fleisch, Karotten und Gewürzen',
        it: 'Traditional pilaf made with rice, meat, carrots, and spices',
        pl: 'Traditional pilaf made with rice, meat, carrots, and spices',
        nl: 'Traditional pilaf made with rice, meat, carrots, and spices',
        pt: 'Traditional pilaf made with rice, meat, carrots, and spices',
      },
      cookingTime: 120,
      difficulty: 'hard',
      servings: 8,
      dietType: {
        en: 'meat',
        sv: 'kött',
        es: 'carne',
        fr: 'viande',
        de: 'Fleisch',
        it: 'meat',
        pl: 'meat',
        nl: 'meat',
        pt: 'meat',
      },
      ingredients: [
        {
          name: {
            en: 'Lamb or beef',
            sv: 'Lamm eller nötkött',
            es: 'Cordero o ternera',
            fr: 'Agneau ou bœuf',
            de: 'Lamm oder Rind',
            it: 'Lamb or beef',
            pl: 'Lamb or beef',
            nl: 'Lamb or beef',
            pt: 'Lamb or beef',
          },
          amount: 800,
          unit: 'g',
        },
        {
          name: {
            en: 'Rice',
            sv: 'Ris',
            es: 'Arroz',
            fr: 'Riz',
            de: 'Reis',
            it: 'Rice',
            pl: 'Rice',
            nl: 'Rice',
            pt: 'Rice',
          },
          amount: 600,
          unit: 'g',
        },
        {
          name: {
            en: 'Carrots',
            sv: 'Morötter',
            es: 'Zanahorias',
            fr: 'Carottes',
            de: 'Karotten',
            it: 'Carrots',
            pl: 'Carrots',
            nl: 'Carrots',
            pt: 'Carrots',
          },
          amount: 4,
          unit: {
            en: 'whole',
            sv: 'hela',
            es: 'entero',
            fr: 'entier',
            de: 'ganz',
            it: 'whole',
            pl: 'whole',
            nl: 'whole',
            pt: 'whole',
          },
        },
        {
          name: {
            en: 'Onions',
            sv: 'Lök',
            es: 'Cebollas',
            fr: 'Oignons',
            de: 'Zwiebeln',
            it: 'Onions',
            pl: 'Onions',
            nl: 'Onions',
            pt: 'Onions',
          },
          amount: 2,
          unit: {
            en: 'whole',
            sv: 'hela',
            es: 'entero',
            fr: 'entier',
            de: 'ganz',
            it: 'whole',
            pl: 'whole',
            nl: 'whole',
            pt: 'whole',
          },
        },
        {
          name: {
            en: 'Garlic head',
            sv: 'Vitlökshuvud',
            es: 'Cabeza de ajo',
            fr: 'Tête d\'ail',
            de: 'Knoblauchkopf',
            it: 'Garlic head',
            pl: 'Garlic head',
            nl: 'Garlic head',
            pt: 'Garlic head',
          },
          amount: 1,
          unit: {
            en: 'whole',
            sv: 'hela',
            es: 'entero',
            fr: 'entier',
            de: 'ganz',
            it: 'whole',
            pl: 'whole',
            nl: 'whole',
            pt: 'whole',
          },
        },
        {
          name: {
            en: 'Cumin',
            sv: 'Kummin',
            es: 'Comino',
            fr: 'Cumin',
            de: 'Kreuzkümmel',
            it: 'Cumin',
            pl: 'Cumin',
            nl: 'Cumin',
            pt: 'Cumin',
          },
          amount: 1,
          unit: {
            en: 'tbsp',
            sv: 'msk',
            es: 'cucharada',
            fr: 'à soupe',
            de: 'EL',
            it: 'tbsp',
            pl: 'tbsp',
            nl: 'tbsp',
            pt: 'tbsp',
          },
        },
        {
          name: {
            en: 'Vegetable oil',
            sv: 'Vegetabilisk olja',
            es: 'Aceite vegetal',
            fr: 'Huile végétale',
            de: 'Pflanzenöl',
            it: 'Vegetable oil',
            pl: 'Vegetable oil',
            nl: 'Vegetable oil',
            pt: 'Vegetable oil',
          },
          amount: 150,
          unit: 'ml',
        },
        {
          name: {
            en: 'Salt',
            sv: 'Salt',
            es: 'Sal',
            fr: 'Sel',
            de: 'Salz',
            it: 'Salt',
            pl: 'Salt',
            nl: 'Salt',
            pt: 'Salt',
          },
          amount: 1.5,
          unit: 'tsp',
        },
      ],
      steps: [
        {
          en: 'Heat oil in a large heavy pot',
          sv: 'Hetta upp olja i en stor tjock gryta',
          es: 'Calentar aceite en una olla grande y pesada.',
          fr: 'Faire chauffer l\'huile dans une grande marmite épaisse',
          de: 'Öl in einem großen, schweren Topf erhitzen',
          it: 'Heat oil in a large heavy pot',
          pl: 'Heat oil in a large heavy pot',
          nl: 'Heat oil in a large heavy pot',
          pt: 'Heat oil in a large heavy pot',
        },
        {
          en: 'Brown meat pieces until well colored',
          sv: 'Bryn köttbitarna tills de fått fin färg',
          es: 'Dore los trozos de carne hasta que estén bien coloreados',
          fr: 'Faire revenir les morceaux de viande jusqu\'à ce qu\'ils soient bien colorés',
          de: 'Fleischstücke anbraten, bis sie eine schöne Farbe haben',
          it: 'Brown meat pieces until well colored',
          pl: 'Brown meat pieces until well colored',
          nl: 'Brown meat pieces until well colored',
          pt: 'Brown meat pieces until well colored',
        },
        {
          en: 'Add sliced onions and cook until golden',
          sv: 'Lägg i skivad lök och koka tills den är gyllene',
          es: 'Agregue las cebollas en rodajas y cocine hasta que estén doradas',
          fr: 'Ajouter les oignons émincés et cuire jusqu\'à ce qu\'ils soient dorés',
          de: 'In Scheiben geschnittene Zwiebeln hinzufügen und goldbraun braten',
          it: 'Add sliced onions and cook until golden',
          pl: 'Add sliced onions and cook until golden',
          nl: 'Add sliced onions and cook until golden',
          pt: 'Add sliced onions and cook until golden',
        },
        {
          en: 'Add julienned carrots and sauté gently',
          sv: 'Tillsätt julienade morötter och fräs försiktigt',
          es: 'Añade las zanahorias cortadas en juliana y saltea suavemente',
          fr: 'Ajouter les carottes en julienne et faire revenir doucement',
          de: 'Julienne-Karotten dazugeben und leicht anbraten',
          it: 'Add julienned carrots and sauté gently',
          pl: 'Add julienned carrots and sauté gently',
          nl: 'Add julienned carrots and sauté gently',
          pt: 'Add julienned carrots and sauté gently',
        },
        {
          en: 'Season with cumin and salt',
          sv: 'Krydda med spiskummin och salt',
          es: 'Condimentar con comino y sal',
          fr: 'Assaisonner de cumin et de sel',
          de: 'Mit Kreuzkümmel und Salz würzen',
          it: 'Season with cumin and salt',
          pl: 'Season with cumin and salt',
          nl: 'Season with cumin and salt',
          pt: 'Season with cumin and salt',
        },
        {
          en: 'Add enough water to cover ingredients and simmer',
          sv: 'Tillsätt tillräckligt med vatten för att täcka ingredienserna och låt sjuda',
          es: 'Agregue suficiente agua para cubrir los ingredientes y cocine a fuego lento',
          fr: 'Ajouter suffisamment d\'eau pour couvrir les ingrédients et laisser mijoter',
          de: 'So viel Wasser hinzufügen, dass die Zutaten bedeckt sind, und köcheln lassen',
          it: 'Add enough water to cover ingredients and simmer',
          pl: 'Add enough water to cover ingredients and simmer',
          nl: 'Add enough water to cover ingredients and simmer',
          pt: 'Add enough water to cover ingredients and simmer',
        },
        {
          en: 'Rinse rice thoroughly and layer it on top without stirring',
          sv: 'Skölj riset noggrant och lägg det ovanpå utan att röra',
          es: 'Enjuague bien el arroz y colóquelo encima sin revolver',
          fr: 'Rincez soigneusement le riz et posez-le dessus sans remuer',
          de: 'Reis gründlich abspülen und ohne Rühren darauf schichten',
          it: 'Rinse rice thoroughly and layer it on top without stirring',
          pl: 'Rinse rice thoroughly and layer it on top without stirring',
          nl: 'Rinse rice thoroughly and layer it on top without stirring',
          pt: 'Rinse rice thoroughly and layer it on top without stirring',
        },
        {
          en: 'Place whole garlic head into the rice',
          sv: 'Lägg hela vitlökshuvudet i riset',
          es: 'Coloca la cabeza de ajo entera en el arroz',
          fr: 'Placer la tête d\'ail entière dans le riz',
          de: 'Den ganzen Knoblauchkopf in den Reis geben',
          it: 'Place whole garlic head into the rice',
          pl: 'Place whole garlic head into the rice',
          nl: 'Place whole garlic head into the rice',
          pt: 'Place whole garlic head into the rice',
        },
        {
          en: 'Cover and steam until rice is tender and liquid absorbed',
          sv: 'Täck över och ånga tills riset är mört och vätskan absorberats',
          es: 'Cubra y cocine al vapor hasta que el arroz esté tierno y se absorba el líquido',
          fr: 'Couvrir et cuire à la vapeur jusqu\'à ce que le riz soit tendre et que le liquide soit absorbé',
          de: 'Abdecken und dämpfen, bis der Reis weich ist und die Flüssigkeit aufgesogen ist',
          it: 'Cover and steam until rice is tender and liquid absorbed',
          pl: 'Cover and steam until rice is tender and liquid absorbed',
          nl: 'Cover and steam until rice is tender and liquid absorbed',
          pt: 'Cover and steam until rice is tender and liquid absorbed',
        },
        {
          en: 'Gently mix before serving and serve family-style',
          sv: 'Blanda försiktigt före servering och servera i familjestil',
          es: 'Mezcle suavemente antes de servir y sirva al estilo familiar',
          fr: 'Mélanger délicatement avant de servir et servir en famille',
          de: 'Vor dem Servieren vorsichtig mischen und im Familienstil servieren',
          it: 'Gently mix before serving and serve family-style',
          pl: 'Gently mix before serving and serve family-style',
          nl: 'Gently mix before serving and serve family-style',
          pt: 'Gently mix before serving and serve family-style',
        },
      ],
      imageUrl: 'https://images.pexels.com/photos/14693845/pexels-photo-14693845.jpeg?auto=compress&cs=tinysrgb&h=650&w=940',
    },
    dessert: {
      id: 'tajikistan-dessert',
      name: {
        en: 'Halva',
        sv: 'Halva',
        es: 'Halva',
        fr: 'Halva',
        de: 'Halva',
        it: 'Halva',
        pl: 'Halva',
        nl: 'Halva',
        pt: 'Halva',
      },
      description: {
        en: 'A traditional Tajik sweet made from flour, oil, sugar, and nuts, often prepared for celebrations and guests.',
        sv: 'En traditionell tadzjikisk sötsak gjord av mjöl, olja, socker och nötter, ofta förberedd för fester och gäster.',
        es: 'Un dulce tradicional tayiko elaborado con harina, aceite, azúcar y nueces, que a menudo se prepara para celebraciones e invitados.',
        fr: 'Une friandise tadjike traditionnelle à base de farine, d\'huile, de sucre et de noix, souvent préparée pour les célébrations et les invités.',
        de: 'Eine traditionelle tadschikische Süßigkeit aus Mehl, Öl, Zucker und Nüssen, die oft für Feiern und Gäste zubereitet wird.',
        it: 'A traditional Tajik sweet made from flour, oil, sugar, and nuts, often prepared for celebrations and guests.',
        pl: 'A traditional Tajik sweet made from flour, oil, sugar, and nuts, often prepared for celebrations and guests.',
        nl: 'A traditional Tajik sweet made from flour, oil, sugar, and nuts, often prepared for celebrations and guests.',
        pt: 'A traditional Tajik sweet made from flour, oil, sugar, and nuts, often prepared for celebrations and guests.',
      },
      cookingTime: 30,
      difficulty: 'easy',
      servings: 6,
      dietType: {
        en: 'vegetarian',
        sv: 'vegetarisk',
        es: 'vegetariano',
        fr: 'végétarien',
        de: 'vegetarisch',
        it: 'vegetarian',
        pl: 'vegetarian',
        nl: 'vegetarian',
        pt: 'vegetarian',
      },
      ingredients: [
        {
          name: {
            en: 'All-purpose flour',
            sv: 'Mjöl för alla ändamål',
            es: 'Harina para todo uso',
            fr: 'Farine tout usage',
            de: 'Allzweckmehl',
            it: 'All-purpose flour',
            pl: 'All-purpose flour',
            nl: 'All-purpose flour',
            pt: 'All-purpose flour',
          },
          amount: 200,
          unit: 'g',
        },
        {
          name: {
            en: 'Vegetable oil or butter',
            sv: 'Vegetabilisk olja eller smör',
            es: 'Aceite vegetal o mantequilla',
            fr: 'Huile végétale ou beurre',
            de: 'Pflanzenöl oder Butter',
            it: 'Vegetable oil or butter',
            pl: 'Vegetable oil or butter',
            nl: 'Vegetable oil or butter',
            pt: 'Vegetable oil or butter',
          },
          amount: 120,
          unit: 'ml',
        },
        {
          name: {
            en: 'Sugar',
            sv: 'Socker',
            es: 'Azúcar',
            fr: 'Sucre',
            de: 'Zucker',
            it: 'Sugar',
            pl: 'Sugar',
            nl: 'Sugar',
            pt: 'Sugar',
          },
          amount: 150,
          unit: 'g',
        },
        {
          name: {
            en: 'Water',
            sv: 'Vatten',
            es: 'Agua',
            fr: 'Eau',
            de: 'Wasser',
            it: 'Water',
            pl: 'Water',
            nl: 'Water',
            pt: 'Water',
          },
          amount: 200,
          unit: 'ml',
        },
        {
          name: {
            en: 'Crushed walnuts or almonds',
            sv: 'Krossade valnötter eller mandel',
            es: 'Nueces o almendras trituradas',
            fr: 'Noix ou amandes concassées',
            de: 'Zerkleinerte Walnüsse oder Mandeln',
            it: 'Crushed walnuts or almonds',
            pl: 'Crushed walnuts or almonds',
            nl: 'Crushed walnuts or almonds',
            pt: 'Crushed walnuts or almonds',
          },
          amount: 80,
          unit: 'g',
        },
        {
          name: {
            en: 'Cardamom',
            sv: 'Kardemumma',
            es: 'Cardamomo',
            fr: 'Cardamome',
            de: 'Kardamom',
            it: 'Cardamom',
            pl: 'Cardamom',
            nl: 'Cardamom',
            pt: 'Cardamom',
          },
          amount: 0.5,
          unit: 'tsp',
        },
      ],
      steps: [
        {
          en: 'Heat oil in a pan over medium heat',
          sv: 'Värm olja i en panna på medelvärme',
          es: 'Calienta aceite en una sartén a fuego medio',
          fr: 'Chauffer l\'huile dans une poêle à feu moyen',
          de: 'Öl in einer Pfanne bei mittlerer Hitze erhitzen',
          it: 'Heat oil in a pan over medium heat',
          pl: 'Heat oil in a pan over medium heat',
          nl: 'Heat oil in a pan over medium heat',
          pt: 'Heat oil in a pan over medium heat',
        },
        {
          en: 'Add flour and stir continuously until golden brown',
          sv: 'Tillsätt mjöl och rör hela tiden tills det är gyllenbrunt',
          es: 'Agrega la harina y revuelve continuamente hasta que se dore.',
          fr: 'Ajouter la farine et remuer continuellement jusqu\'à ce qu\'elle soit dorée',
          de: 'Mehl hinzufügen und unter ständigem Rühren goldbraun rühren',
          it: 'Add flour and stir continuously until golden brown',
          pl: 'Add flour and stir continuously until golden brown',
          nl: 'Add flour and stir continuously until golden brown',
          pt: 'Add flour and stir continuously until golden brown',
        },
        {
          en: 'In a separate pot, dissolve sugar in water and bring to a boil',
          sv: 'Lös socker i vatten i en separat kastrull och koka upp',
          es: 'En una olla aparte, disolver el azúcar en agua y llevar a ebullición',
          fr: 'Dans une autre casserole, dissoudre le sucre dans l\'eau et porter à ébullition',
          de: 'In einem separaten Topf Zucker in Wasser auflösen und zum Kochen bringen',
          it: 'In a separate pot, dissolve sugar in water and bring to a boil',
          pl: 'In a separate pot, dissolve sugar in water and bring to a boil',
          nl: 'In a separate pot, dissolve sugar in water and bring to a boil',
          pt: 'In a separate pot, dissolve sugar in water and bring to a boil',
        },
        {
          en: 'Carefully pour syrup into flour mixture while stirring',
          sv: 'Häll försiktigt sirap i mjölblandningen under omrörning',
          es: 'Vierta con cuidado el almíbar en la mezcla de harina mientras revuelve',
          fr: 'Versez délicatement le sirop dans le mélange de farine tout en remuant',
          de: 'Den Sirup vorsichtig unter Rühren in die Mehlmischung gießen',
          it: 'Carefully pour syrup into flour mixture while stirring',
          pl: 'Carefully pour syrup into flour mixture while stirring',
          nl: 'Carefully pour syrup into flour mixture while stirring',
          pt: 'Carefully pour syrup into flour mixture while stirring',
        },
        {
          en: 'Add cardamom and mix until thick and smooth',
          sv: 'Tillsätt kardemumma och blanda tills det är tjockt och slätt',
          es: 'Agregue el cardamomo y mezcle hasta que esté espeso y suave',
          fr: 'Ajouter la cardamome et mélanger jusqu\'à consistance épaisse et lisse',
          de: 'Kardamom hinzufügen und verrühren, bis es dick und glatt ist',
          it: 'Add cardamom and mix until thick and smooth',
          pl: 'Add cardamom and mix until thick and smooth',
          nl: 'Add cardamom and mix until thick and smooth',
          pt: 'Add cardamom and mix until thick and smooth',
        },
        {
          en: 'Stir in crushed nuts',
          sv: 'Stir in crushed nuts',
          es: 'Agregue las nueces trituradas',
          fr: 'Incorporer les noix concassées',
          de: 'Gemahlene Nüsse unterrühren',
          it: 'Stir in crushed nuts',
          pl: 'Stir in crushed nuts',
          nl: 'Stir in crushed nuts',
          pt: 'Stir in crushed nuts',
        },
        {
          en: 'Cook for 2–3 minutes until glossy',
          sv: 'Koka i 2–3 minuter tills det blir glansigt',
          es: 'Cocine durante 2 a 3 minutos hasta que esté brillante',
          fr: 'Cuire pendant 2 à 3 minutes jusqu\'à ce que ce soit brillant',
          de: '2–3 Minuten kochen, bis es glänzt',
          it: 'Cook for 2–3 minutes until glossy',
          pl: 'Cook for 2–3 minutes until glossy',
          nl: 'Cook for 2–3 minutes until glossy',
          pt: 'Cook for 2–3 minutes until glossy',
        },
        {
          en: 'Transfer to a dish and smooth the surface',
          sv: 'Överför till ett fat och jämna till ytan',
          es: 'Transfiera a un plato y alise la superficie',
          fr: 'Transférer dans un plat et lisser la surface',
          de: 'In eine Schüssel geben und die Oberfläche glatt streichen',
          it: 'Transfer to a dish and smooth the surface',
          pl: 'Transfer to a dish and smooth the surface',
          nl: 'Transfer to a dish and smooth the surface',
          pt: 'Transfer to a dish and smooth the surface',
        },
        {
          en: 'Let cool slightly before serving',
          sv: 'Låt svalna något innan servering',
          es: 'Dejar enfriar un poco antes de servir',
          fr: 'Laisser refroidir légèrement avant de servir',
          de: 'Vor dem Servieren etwas abkühlen lassen',
          it: 'Let cool slightly before serving',
          pl: 'Let cool slightly before serving',
          nl: 'Let cool slightly before serving',
          pt: 'Let cool slightly before serving',
        },
      ],
      imageUrl: 'https://images.pexels.com/photos/25388881/pexels-photo-25388881.jpeg?auto=compress&cs=tinysrgb&h=650&w=940',
    },
    drinks: {
      alcoholic: 'Vodka',
      nonAlcoholic: 'Green tea, Ayran, Kumis (fermented milk)',
    },
    music: [
      { en: 'Shashmaqam (classical tradition)', sv: 'Shashmaqam (klassisk tradition)', de: 'Shashmaqam (klassische Tradition)', fr: 'Shashmaqam (tradition classique)', es: 'Shashmaqam (tradición clásica)', it: 'Shashmaqam (tradizione classica)', pl: 'Shashmaqam (tradycja klasyczna)', nl: 'Shashmaqam (klassieke traditie)', pt: 'Shashmaqam (tradição clássica)' },
      { en: 'Folk music with daf drum', sv: 'Folkmusik med daff-trumma', de: 'Volksmusik mit Daf-Trommel', fr: 'Musique folklorique avec tambour daf', es: 'Música folclórica con tambor daf', it: 'Musica folk con tamburo daf', pl: 'Muzyka ludowa z bębnem daf', nl: 'Volksmuziek met daf-drum', pt: 'Música folclórica com tambor daf' },
      { en: 'Persian-influenced melodies', sv: 'Persisk-inspirerade melodier', de: 'Persisch beeinflusste Melodien', fr: 'Mélodies d\'influence persane', es: 'Melodías de influencia persa', it: 'Melodie di influenza persiana', pl: 'Melodie z wpływami perskimi', nl: 'Perzisch beïnvloede melodieën', pt: 'Melodias de influência persa' },
    ],
    decorationIdeas: [
      { en: 'Red, white, and green flag colors', sv: 'Röd, vit och grön flaggfärger', de: 'Rote, weiße und grüne Flaggenfarben', fr: 'Couleurs du drapeau : rouge, blanc et vert', es: 'Colores de la bandera: rojo, blanco y verde', it: 'Colori della bandiera: rosso, bianco e verde', pl: 'Kolory flagi: czerwony, biały i zielony', nl: 'Rode, witte en groene vlagkleuren', pt: 'Cores da bandeira: vermelho, branco e verde' },
      { en: 'Mountain and glacier imagery', sv: 'Berg och glaciärbilder', de: 'Berg- und Gletscherbilder', fr: 'Imagerie de montagne et de glacier', es: 'Imágenes de montañas y glaciares', it: 'Immagini di montagne e ghiacciai', pl: 'Obrazy gór i lodowców', nl: 'Berg- en gletsjersbeelden', pt: 'Imagens de montanhas e glaciares' },
      { en: 'Silk Road motifs', sv: 'Sidenvägens motiv', de: 'Seidenstraßen-Motive', fr: 'Motifs de la Route de la Soie', es: 'Motivos de la Ruta de la Seda', it: 'Motivi della Via della Seta', pl: 'Motywy Jedwabnego Szlaku', nl: 'Zijderoute motieven', pt: 'Motivos da Rota da Seda' },
      { en: 'Traditional textiles and patterns', sv: 'Traditionella textilier och mönster', de: 'Traditionelle Textilien und Muster', fr: 'Textiles et motifs traditionnels', es: 'Textiles y patrones tradicionales', it: 'Tessuti e motivi tradizionali', pl: 'Tradycyjne tkaniny i wzory', nl: 'Traditionele textiel en patronen', pt: 'Têxteis e padrões tradicionais' },
      { en: 'Carpet-inspired table runners and embroidered cushions', sv: 'Mattinspirerade bordslöpare och broderade kuddar', de: 'Teppich-inspirierte Tischläufer und bestickte Kissen', fr: 'Chemins de table inspirés des tapis et coussins brodés', es: 'Caminos de mesa inspirados en alfombras y cojines bordados', it: 'Corritori da tavola ispirati ai tappeti e cuscini ricamati', pl: 'Bieżniki inspirowane dywanami i haftowane poduszki', nl: 'Tapijt-geïnspireerde tafellopers en geborduurde kussens', pt: 'Trilhos de mesa inspirados em tapetes e almofadas bordadas' },
    ],
    conversationStarters: [
      { en: 'Would you dare to travel the Pamir Highway?', sv: 'Skulle du våga resa Pamirhöglandsvägen?', de: 'Würden Sie wagen, den Pamir Highway zu bereisen?', fr: 'Oseriez-vous voyager sur la route du Pamir ?', es: '¿Te atreverías a viajar por la Autopista del Pamir?', it: 'Oseresti percorrere la Pamir Highway?', pl: 'Czy odważyłbyś się podróżować Autostradą Pamirską?', nl: 'Zou je het aandurven de Pamir Highway te reizen?', pt: 'Você se atreveria a percorrer a Rodovia Pamir?' },
      { en: 'What do you know about the Silk Road?', sv: 'Vad vet du om Sidenvägen?', de: 'Was wissen Sie über die Seidenstraße?', fr: 'Que savez-vous de la Route de la Soie ?', es: '¿Qué sabes sobre la Ruta de la Seda?', it: 'Cosa sai della Via della Seta?', pl: 'Co wiesz o Jedwabnym Szlaku?', nl: 'Wat weet u over de Zijderoute?', pt: 'O que você sabe sobre a Rota da Seda?' },
      { en: 'Have you tried Central Asian pilaf before?', sv: 'Har du provat centralasiatisk pilaf förut?', de: 'Haben Sie schon zentralasiatischen Pilaw probiert?', fr: 'Avez-vous déjà essayé le pilaf d\'Asie centrale ?', es: '¿Has probado el pilaf de Asia Central antes?', it: 'Hai mai provato il pilaf centroasiatico?', pl: 'Czy próbowałeś środkowoazjatyckiego pilafu wcześniej?', nl: 'Heb je al eerder Centraal-Aziatische pilaf geprobeerd?', pt: 'Você já experimentou o pilaf da Ásia Central?' },
      { en: 'Did you know Tajik is closely related to Persian?', sv: 'Visste du att tadzjikiska är nära besläktad med persiska?', de: 'Wussten Sie, dass Tadschikisch eng mit Persisch verwandt ist?', fr: 'Saviez-vous que le tadjik est étroitement lié au persan ?', es: '¿Sabías que el tayiko está estrechamente relacionado con el persa?', it: 'Sapevi che il tagico è strettamente correlato al persiano?', pl: 'Czy wiedziałeś, że tadżycki jest blisko spokrewniony z perskim?', nl: 'Wist je dat Tadzjieks nauw verwant is aan Perzisch?', pt: 'Você sabia que o tajique está intimamente relacionado ao persa?' },
      { en: 'Are you more drawn to mountains, culture, or food when you travel?', sv: 'Lockas du mer av berg, kultur eller mat när du reser?', de: 'Werden Sie mehr von Bergen, Kultur oder Essen angezogen, wenn Sie reisen?', fr: 'Êtes-vous plus attiré par les montagnes, la culture ou la nourriture lorsque vous voyagez ?', es: '¿Te atrae más las montañas, la cultura o la comida cuando viajas?', it: 'Sei più attratto da montagne, cultura o cibo quando viaggi?', pl: 'Czy bardziej pociągają cię góry, kultura czy jedzenie podczas podróży?', nl: 'Word je meer aangetrokken door bergen, cultuur of eten als je reist?', pt: 'Você se sente mais atraído por montanhas, cultura ou comida quando viaja?' },
    ],
    quiz: [
      {
        id: 'tajikistan-q1',
        question: {
          en: 'What is the capital of Tajikistan?',
          sv: 'Vad är huvudstaden i Tadzjikistan?',
          es: '¿Cuál es la capital de Tayikistán?',
          fr: 'Quelle est la capitale du Tadjikistan ?',
          de: 'Was ist die Hauptstadt Tadschikistans?',
          it: 'What is the capital of Tajikistan?',
          pl: 'What is the capital of Tajikistan?',
          nl: 'What is the capital of Tajikistan?',
          pt: 'What is the capital of Tajikistan?',
        },
        options: [
          {
            en: 'Dushanbe',
            sv: 'Dushanbe',
            es: 'Dusambé',
            fr: 'Douchanbé',
            de: 'Duschanbe',
            it: 'Dushanbe',
            pl: 'Dushanbe',
            nl: 'Dushanbe',
            pt: 'Dushanbe',
          },
          {
            en: 'Bishkek',
            sv: 'Bisjkek',
            es: 'Biskek',
            fr: 'Bichkek',
            de: 'Bischkek',
            it: 'Bishkek',
            pl: 'Bishkek',
            nl: 'Bishkek',
            pt: 'Bishkek',
          },
          {
            en: 'Tashkent',
            sv: 'Tasjkent',
            es: 'Taskent',
            fr: 'Tachkent',
            de: 'Taschkent',
            it: 'Tashkent',
            pl: 'Tashkent',
            nl: 'Tashkent',
            pt: 'Tashkent',
          },
          {
            en: 'Ashgabat',
            sv: 'Ashgabat',
            es: 'Asjabad',
            fr: 'Achgabat',
            de: 'Aschgabat',
            it: 'Ashgabat',
            pl: 'Ashgabat',
            nl: 'Ashgabat',
            pt: 'Ashgabat',
          },
        ],
        correctAnswer: 0,
      },
      {
        id: 'tajikistan-q2',
        question: {
          en: 'Which mountain range dominates Tajikistan?',
          sv: 'Vilken bergskedja dominerar Tadzjikistan?',
          es: '¿Qué cadena montañosa domina Tayikistán?',
          fr: 'Quelle chaîne de montagnes domine le Tadjikistan ?',
          de: 'Welches Gebirge dominiert Tadschikistan?',
          it: 'Which mountain range dominates Tajikistan?',
          pl: 'Which mountain range dominates Tajikistan?',
          nl: 'Which mountain range dominates Tajikistan?',
          pt: 'Which mountain range dominates Tajikistan?',
        },
        options: [
          {
            en: 'Himalayas',
            sv: 'Himalaya',
            es: 'Himalaya',
            fr: 'Himalaya',
            de: 'Himalaya',
            it: 'Himalayas',
            pl: 'Himalayas',
            nl: 'Himalayas',
            pt: 'Himalayas',
          },
          {
            en: 'Pamir Mountains',
            sv: 'Pamir Mountains',
            es: 'Montañas Pamir',
            fr: 'Montagnes du Pamir',
            de: 'Pamir-Gebirge',
            it: 'Pamir Mountains',
            pl: 'Pamir Mountains',
            nl: 'Pamir Mountains',
            pt: 'Pamir Mountains',
          },
          {
            en: 'Caucasus',
            sv: 'Kaukasus',
            es: 'Cáucaso',
            fr: 'Caucase',
            de: 'Kaukasus',
            it: 'Caucasus',
            pl: 'Caucasus',
            nl: 'Caucasus',
            pt: 'Caucasus',
          },
          {
            en: 'Ural Mountains',
            sv: 'Ural Mountains',
            es: 'Montes Urales',
            fr: 'Montagnes de l\'Oural',
            de: 'Uralgebirge',
            it: 'Ural Mountains',
            pl: 'Ural Mountains',
            nl: 'Ural Mountains',
            pt: 'Ural Mountains',
          },
        ],
        correctAnswer: 1,
      },
      {
        id: 'tajikistan-q3',
        question: {
          en: 'What ancient trade route passed through Tajikistan?',
          sv: 'Vilken forntida handelsväg gick genom Tadzjikistan?',
          es: '¿Qué antigua ruta comercial pasaba por Tayikistán?',
          fr: 'Quelle ancienne route commerciale traversait le Tadjikistan ?',
          de: 'Welche alte Handelsroute führte durch Tadschikistan?',
          it: 'What ancient trade route passed through Tajikistan?',
          pl: 'What ancient trade route passed through Tajikistan?',
          nl: 'What ancient trade route passed through Tajikistan?',
          pt: 'What ancient trade route passed through Tajikistan?',
        },
        options: [
          {
            en: 'Amber Road',
            sv: 'Amber Road',
            es: 'Camino ámbar',
            fr: 'Route de l\'Ambre',
            de: 'Bernsteinstraße',
            it: 'Amber Road',
            pl: 'Amber Road',
            nl: 'Amber Road',
            pt: 'Amber Road',
          },
          {
            en: 'Spice Route',
            sv: 'Kryddväg',
            es: 'Ruta de las Especias',
            fr: 'Route des épices',
            de: 'Gewürzroute',
            it: 'Spice Route',
            pl: 'Spice Route',
            nl: 'Spice Route',
            pt: 'Spice Route',
          },
          {
            en: 'Silk Road',
            sv: 'Silk Road',
            es: 'Ruta de la Seda',
            fr: 'Route de la Soie',
            de: 'Seidenstraße',
            it: 'Silk Road',
            pl: 'Silk Road',
            nl: 'Silk Road',
            pt: 'Silk Road',
          },
          {
            en: 'Tea Horse Road',
            sv: 'Tea Horse Road',
            es: 'Ruta del té y los caballos',
            fr: 'Route du cheval du thé',
            de: 'Tee-Pferde-Straße',
            it: 'Tea Horse Road',
            pl: 'Tea Horse Road',
            nl: 'Tea Horse Road',
            pt: 'Tea Horse Road',
          },
        ],
        correctAnswer: 2,
      },
      {
        id: 'tajikistan-q4',
        question: {
          en: 'Over what percentage of Tajikistan is mountainous?',
          sv: 'Över hur stor andel av Tadzjikistan är bergigt?',
          es: '¿Sobre qué porcentaje de Tayikistán es montañoso?',
          fr: 'Sur quelle proportion du Tadjikistan est-il montagneux ?',
          de: 'Über wie viel Prozent der Fläche Tadschikistans ist gebirgig?',
          it: 'Over what percentage of Tajikistan is mountainous?',
          pl: 'Over what percentage of Tajikistan is mountainous?',
          nl: 'Over what percentage of Tajikistan is mountainous?',
          pt: 'Over what percentage of Tajikistan is mountainous?',
        },
        options: [
          {
            en: '30%',
            sv: '30 %',
            es: '30%',
            fr: '30%',
            de: '30 %',
            it: '30%',
            pl: '30%',
            nl: '30%',
            pt: '30%',
          },
          {
            en: '50%',
            sv: '50 %',
            es: '50%',
            fr: '50%',
            de: '50 %',
            it: '50%',
            pl: '50%',
            nl: '50%',
            pt: '50%',
          },
          {
            en: '70%',
            sv: '70 %',
            es: '70%',
            fr: '70%',
            de: '70 %',
            it: '70%',
            pl: '70%',
            nl: '70%',
            pt: '70%',
          },
          {
            en: '90%',
            sv: '90 %',
            es: '90%',
            fr: '90%',
            de: '90 %',
            it: '90%',
            pl: '90%',
            nl: '90%',
            pt: '90%',
          },
        ],
        correctAnswer: 3,
      },
      {
        id: 'tajikistan-q5',
        question: {
          en: 'What language is most closely related to Tajik?',
          sv: 'Vilket språk är närmast besläktat med tadzjikiska?',
          es: '¿Qué idioma está más estrechamente relacionado con el tayiko?',
          fr: 'Quelle langue est la plus étroitement liée au tadjik ?',
          de: 'Welche Sprache ist am engsten mit Tadschikisch verwandt?',
          it: 'What language is most closely related to Tajik?',
          pl: 'What language is most closely related to Tajik?',
          nl: 'What language is most closely related to Tajik?',
          pt: 'What language is most closely related to Tajik?',
        },
        options: [
          {
            en: 'Turkish',
            sv: 'turkiska',
            es: 'Turco',
            fr: 'Turc',
            de: 'Türkisch',
            it: 'Turkish',
            pl: 'Turkish',
            nl: 'Turkish',
            pt: 'Turkish',
          },
          {
            en: 'Arabic',
            sv: 'Arabiska',
            es: 'Árabe',
            fr: 'Arabe',
            de: 'Arabisch',
            it: 'Arabic',
            pl: 'Arabic',
            nl: 'Arabic',
            pt: 'Arabic',
          },
          {
            en: 'Persian',
            sv: 'persiska',
            es: 'persa',
            fr: 'persan',
            de: 'Persisch',
            it: 'Persian',
            pl: 'Persian',
            nl: 'Persian',
            pt: 'Persian',
          },
          {
            en: 'Russian',
            sv: 'Ryska',
            es: 'ruso',
            fr: 'Russe',
            de: 'Russisch',
            it: 'Russian',
            pl: 'Russian',
            nl: 'Russian',
            pt: 'Russian',
          },
        ],
        correctAnswer: 2,
      },
      {
        id: 'tajikistan-q6',
        question: {
          en: 'What is the national dish Osh primarily made from?',
          sv: 'Vad är nationalrätten Osh främst gjord av?',
          es: '¿De qué se elabora principalmente el plato nacional Osh?',
          fr: 'De quoi est principalement composé le plat national Och ?',
          de: 'Woraus besteht das Nationalgericht Osch hauptsächlich?',
          it: 'What is the national dish Osh primarily made from?',
          pl: 'What is the national dish Osh primarily made from?',
          nl: 'What is the national dish Osh primarily made from?',
          pt: 'What is the national dish Osh primarily made from?',
        },
        options: [
          {
            en: 'Rice and fish',
            sv: 'Ris och fisk',
            es: 'Arroz y pescado',
            fr: 'Riz et poisson',
            de: 'Reis und Fisch',
            it: 'Rice and fish',
            pl: 'Rice and fish',
            nl: 'Rice and fish',
            pt: 'Rice and fish',
          },
          {
            en: 'Rice and meat',
            sv: 'Ris och kött',
            es: 'Arroz y carne',
            fr: 'Riz et viande',
            de: 'Reis und Fleisch',
            it: 'Rice and meat',
            pl: 'Rice and meat',
            nl: 'Rice and meat',
            pt: 'Rice and meat',
          },
          {
            en: 'Bread and cheese',
            sv: 'Bröd och ost',
            es: 'Pan y queso',
            fr: 'Pain et fromage',
            de: 'Brot und Käse',
            it: 'Bread and cheese',
            pl: 'Bread and cheese',
            nl: 'Bread and cheese',
            pt: 'Bread and cheese',
          },
          {
            en: 'Noodles',
            sv: 'Nudlar',
            es: 'Fideos',
            fr: 'Nouilles',
            de: 'Nudeln',
            it: 'Noodles',
            pl: 'Noodles',
            nl: 'Noodles',
            pt: 'Noodles',
          },
        ],
        correctAnswer: 1,
      },
      {
        id: 'tajikistan-q7',
        question: {
          en: 'What is the Pamir Highway famous for?',
          sv: 'Vad är Pamir Highway känd för?',
          es: '¿Por qué es famosa la autopista Pamir?',
          fr: 'Pourquoi la route du Pamir est-elle célèbre ?',
          de: 'Wofür ist der Pamir Highway berühmt?',
          it: 'What is the Pamir Highway famous for?',
          pl: 'What is the Pamir Highway famous for?',
          nl: 'What is the Pamir Highway famous for?',
          pt: 'What is the Pamir Highway famous for?',
        },
        options: [
          {
            en: 'Being the longest road in Asia',
            sv: 'Att vara den längsta vägen i Asien',
            es: 'Siendo la carretera más larga de Asia',
            fr: 'Être la plus longue route d\'Asie',
            de: 'Die längste Straße Asiens',
            it: 'Being the longest road in Asia',
            pl: 'Being the longest road in Asia',
            nl: 'Being the longest road in Asia',
            pt: 'Being the longest road in Asia',
          },
          {
            en: 'Being one of the highest roads in the world',
            sv: 'Att vara en av de högsta vägarna i världen',
            es: 'Siendo una de las carreteras más altas del mundo',
            fr: 'Étant l\'une des routes les plus hautes du monde',
            de: 'Als eine der höchstgelegenen Straßen der Welt',
            it: 'Being one of the highest roads in the world',
            pl: 'Being one of the highest roads in the world',
            nl: 'Being one of the highest roads in the world',
            pt: 'Being one of the highest roads in the world',
          },
          {
            en: 'Connecting Europe and China directly',
            sv: 'Förbinder Europa och Kina direkt',
            es: 'Conectando Europa y China directamente',
            fr: 'Connecter directement l\'Europe et la Chine',
            de: 'Direkte Verbindung zwischen Europa und China',
            it: 'Connecting Europe and China directly',
            pl: 'Connecting Europe and China directly',
            nl: 'Connecting Europe and China directly',
            pt: 'Connecting Europe and China directly',
          },
          {
            en: 'Crossing a desert',
            sv: 'Att korsa en öken',
            es: 'Cruzando un desierto',
            fr: 'Traversée d\'un désert',
            de: 'Eine Wüste durchqueren',
            it: 'Crossing a desert',
            pl: 'Crossing a desert',
            nl: 'Crossing a desert',
            pt: 'Crossing a desert',
          },
        ],
        correctAnswer: 1,
      },
      {
        id: 'tajikistan-q8',
        question: {
          en: 'Which beverage is commonly consumed with meals in Tajikistan?',
          sv: 'Vilken dryck konsumeras vanligtvis till måltider i Tadzjikistan?',
          es: '¿Qué bebida se consume habitualmente con las comidas en Tayikistán?',
          fr: 'Quelle boisson est couramment consommée avec les repas au Tadjikistan ?',
          de: 'Welches Getränk wird in Tadschikistan häufig zu den Mahlzeiten konsumiert?',
          it: 'Which beverage is commonly consumed with meals in Tajikistan?',
          pl: 'Which beverage is commonly consumed with meals in Tajikistan?',
          nl: 'Which beverage is commonly consumed with meals in Tajikistan?',
          pt: 'Which beverage is commonly consumed with meals in Tajikistan?',
        },
        options: [
          {
            en: 'Green tea',
            sv: 'Grönt te',
            es: 'Té verde',
            fr: 'Thé vert',
            de: 'Grüner Tee',
            it: 'Green tea',
            pl: 'Green tea',
            nl: 'Green tea',
            pt: 'Green tea',
          },
          {
            en: 'Coffee',
            sv: 'Kaffe',
            es: 'Café',
            fr: 'Café',
            de: 'Kaffee',
            it: 'Coffee',
            pl: 'Coffee',
            nl: 'Coffee',
            pt: 'Coffee',
          },
          {
            en: 'Cola',
            sv: 'Cola',
            es: 'cola',
            fr: 'Cola',
            de: 'Cola',
            it: 'Cola',
            pl: 'Cola',
            nl: 'Cola',
            pt: 'Cola',
          },
          {
            en: 'Mate',
            sv: 'Kompis',
            es: 'Compañero',
            fr: 'Compagnon',
            de: 'Kumpel',
            it: 'Mate',
            pl: 'Mate',
            nl: 'Mate',
            pt: 'Mate',
          },
        ],
        correctAnswer: 0,
      },
      {
        id: 'tajikistan-q9',
        question: {
          en: 'Which region is Tajikistan part of?',
          sv: 'Vilken region är Tadzjikistan en del av?',
          es: '¿De qué región forma parte Tayikistán?',
          fr: 'De quelle région fait partie le Tadjikistan ?',
          de: 'Zu welcher Region gehört Tadschikistan?',
          it: 'Which region is Tajikistan part of?',
          pl: 'Which region is Tajikistan part of?',
          nl: 'Which region is Tajikistan part of?',
          pt: 'Which region is Tajikistan part of?',
        },
        options: [
          {
            en: 'Middle East',
            sv: 'Mellanöstern',
            es: 'Medio Oriente',
            fr: 'Moyen-Orient',
            de: 'Naher Osten',
            it: 'Middle East',
            pl: 'Middle East',
            nl: 'Middle East',
            pt: 'Middle East',
          },
          {
            en: 'South Asia',
            sv: 'Sydasien',
            es: 'Asia del Sur',
            fr: 'Asie du Sud',
            de: 'Südasien',
            it: 'South Asia',
            pl: 'South Asia',
            nl: 'South Asia',
            pt: 'South Asia',
          },
          {
            en: 'Central Asia',
            sv: 'Centralasien',
            es: 'Asia Central',
            fr: 'Asie centrale',
            de: 'Zentralasien',
            it: 'Central Asia',
            pl: 'Central Asia',
            nl: 'Central Asia',
            pt: 'Central Asia',
          },
          {
            en: 'East Asia',
            sv: 'Östasien',
            es: 'Asia Oriental',
            fr: 'Asie de l\'Est',
            de: 'Ostasien',
            it: 'East Asia',
            pl: 'East Asia',
            nl: 'East Asia',
            pt: 'East Asia',
          },
        ],
        correctAnswer: 2,
      },
      {
        id: 'tajikistan-q10',
        question: {
          en: 'What instrument is commonly used in Tajik folk music?',
          sv: 'Vilket instrument används vanligtvis i tadzjikisk folkmusik?',
          es: '¿Qué instrumento se utiliza comúnmente en la música folclórica de Tayikistán?',
          fr: 'Quel instrument est couramment utilisé dans la musique folklorique tadjike ?',
          de: 'Welches Instrument wird in der tadschikischen Volksmusik häufig verwendet?',
          it: 'What instrument is commonly used in Tajik folk music?',
          pl: 'What instrument is commonly used in Tajik folk music?',
          nl: 'What instrument is commonly used in Tajik folk music?',
          pt: 'What instrument is commonly used in Tajik folk music?',
        },
        options: [
          {
            en: 'Daf drum',
            sv: 'Daf trumma',
            es: 'Tambor Daf',
            fr: 'Tambour Daf',
            de: 'Daf Trommel',
            it: 'Daf drum',
            pl: 'Daf drum',
            nl: 'Daf drum',
            pt: 'Daf drum',
          },
          {
            en: 'Bagpipes',
            sv: 'Säckpipor',
            es: 'Gaitas',
            fr: 'Cornemuse',
            de: 'Dudelsäcke',
            it: 'Bagpipes',
            pl: 'Bagpipes',
            nl: 'Bagpipes',
            pt: 'Bagpipes',
          },
          {
            en: 'Accordion',
            sv: 'Dragspel',
            es: 'Acordeón',
            fr: 'Accordéon',
            de: 'Akkordeon',
            it: 'Accordion',
            pl: 'Accordion',
            nl: 'Accordion',
            pt: 'Accordion',
          },
          {
            en: 'Sitar',
            sv: 'Sitar',
            es: 'sitar',
            fr: 'Sitar',
            de: 'Sitar',
            it: 'Sitar',
            pl: 'Sitar',
            nl: 'Sitar',
            pt: 'Sitar',
          },
        ],
        correctAnswer: 0,
      },
    ],
    coordinates: {
      x: 67,
      y: 40,
    },
  };
